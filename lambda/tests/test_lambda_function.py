...
def create_test_data(table, days_offset=0):
    ...
    test_data = [
        # 今日の誕生日
        {'id': '1', 'user_name': 'User1', 'birthday': str(today), 'url': 'https://example.com/1'},
        # 1日後の誕生日
        {'id': '2', 'user_name': 'User2', 'birthday': str(today + timedelta(days=1)), 'url': 'https://example.com/2'},
        # 2日後の誕生日
        {'id': '3', 'user_name': 'User3', 'birthday': str(today + timedelta(days=2)), 'url': 'https://example.com/3'},
        # 3日後の誕生日
        {'id': '4', 'user_name': 'User4', 'birthday': str(today + timedelta(days=3)), 'url': 'https://example.com/4'},
        # 4日後の誕生日（範囲外）
        {'id': '5', 'user_name': 'User5', 'birthday': str(today + timedelta(days=4)), 'url': 'https://example.com/5'},
        # 1日前の誕生日（範囲外）
        {'id': '6', 'user_name': 'User6', 'birthday': str(today - timedelta(days=1)), 'url': 'https://example.com/6'}
    ]
...
def test_lambda_handler(dynamodb_table, monkeypatch):
    create_test_data(dynamodb_table)
    from lambda_function import lambda_handler
    ...
    monkeypatch.setattr('discord_webhook.DiscordWebhook.execute', mock_execute)
